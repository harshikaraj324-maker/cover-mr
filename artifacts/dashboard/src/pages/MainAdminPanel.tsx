...
function ChangePinModal({ onClose, onChanged }: { masterPin: string; onClose: () => void; onChanged: (p: string) => void }) {
  ...
  return (
    <Modal onClose={onClose} maxWidth={380}>
      ...
      <form onSubmit={handleSubmit}>
        {[
          { label: "Current PIN", val: curPin, set: setCurPin, show: true },
          { label: "New PIN", val: newPin, set: setNewPin, show: false },
          { label: "Confirm New PIN", val: newPin2, set: setNewPin2, show: false },
        ].map(({ label, val, set }) => (
